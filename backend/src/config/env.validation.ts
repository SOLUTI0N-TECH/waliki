...
export function validate(raw: Record<string, unknown>): EnvConfig {
  ...
  const routerAddress = read('WALIKI_ROUTER_ADDRESS');
  ...
  const missing = Object.entries({
    ...
    // No default on purpose. It used to fall back to the deployed address, and
    // after a redeploy that means settling against a dead contract in silence.
  ...
  if (!isAddress(routerAddress)) {
    invalid.push('WALIKI_ROUTER_ADDRESS (no es una dirección Ethereum válida)');
  }
  if (invalid.length > 0) {
    throw new Error(
